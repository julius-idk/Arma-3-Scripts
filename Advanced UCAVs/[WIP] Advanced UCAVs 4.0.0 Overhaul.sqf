...
_EnableScript = {
	...
	if (isNil "AUCAVs_camouflageCoef") then {
		missionNamespace setVariable ["AUCAVs_camouflageCoef", 0.7, true];
	};
	if (isNil "AUCAVs_audibleCoef") then {
		missionNamespace setVariable ["AUCAVs_audibleCoef", 0.7, true];
	};	
	...
	[[],{
		...
		[] call AdvancedUCAVs_InitOnPlayer_fnc;
...
_DisableScript = {
...

	{
		...
		player setVariable ["JammingOn", false];	
		...
		switch (_type) do {
			...
			case "SLIDER_SPOT": {
				...
				_slider ctrlAddEventHandler ["SliderPosChanged", {
					...
					_valueTxt ctrlSetText ((sliderPosition _slider) toFixed 1);
				}];							
				...
				_button ctrlAddEventHandler ["ButtonClick", {
					...
					["VISIBLITY"] call AdvancedUCAVs_ToggleConfigValues_fnc;
					playSoundUI ["addItemOK"];
				}];							
			};
		};
...
["SLIDER_SPOT", ["CamouflageCoef (visual)", "AUCAVs_camouflageCoef", "Reduces the ability for only AI to spot AR-2s, AL-6s and ED-1s. Lower value means harder to spot.\nDoes only affect spotting, not accuracy\n\nArma Default: 1\nScript Default: 0.7"]],
["SLIDER_SPOT", ["AudibleCoef (sound)", "AUCAVs_audibleCoef", "Reduces the ability for only AI to hear AR-2s, AL-6s and ED-1s. Lower value means harder to hear.\nDoes only affect hearing, not accuracy\n\nArma Default: 1\nScript Default: 0.7"]]	
	];
};
...
AdvancedUCAVs_InitOnPlayer_fnc = {
	...
	[
		...
		"<font color='#FF0000'>! Keep in mind that this can also jamm friendly drones. Be especially careful with the backpack !</font><br/><br/><br/><br/>" +
	...
	AdvancedUCAVs_EachFrameEH = addMissionEventHandler ["EachFrame", {	
		if (diag_frameNo < AdvancedUCAVs_timePlusTime) exitWith {};
		AdvancedUCAVs_timePlusTime = time + 0.01;
	...
	AdvancedUCAVs_ChatCommandMissionEH = addMissionEventHandler ["HandleChatMessage", {
		...
		if (((toLower _message) find "!ucav_config") == 0) then {
			...
			if ((str getAssignedCuratorLogic player) == "bis_curator_1" && { ["IsSpectating"] call BIS_fnc_EGSpectator }) exitWith { 
				[] spawn { sleep 0.01; systemChat "[Advanced UCAVs] Sorry, but it apears that the game moderator slot is disabled." }; 
			};			
	...
	AdvancedUCAVs_RespawnEH = player addEventHandler ["Respawn", {
		...
		[] spawn AdvancedUCAVs_AddKeybinds_fnc;
		_unit setVariable ["JammingOn", false];
	...
	AdvancedUCAVs_BackpackJamming_fnc = {
		...
		_isJamming = player getVariable ["JammingOn", false];		
		if (_isJamming) then {	
			...
			player setVariable ["JammingOn", true];
			
			while { player getVariable ["JammingOn", false] } do {
				...
				if (alive player) then {
					...
					_nearSmallUAVs = (getPos player) nearEntities [["UAV_01_base_F", "UAV_06_base_F"], 200];						
					_nearSmallUGVs = (getPos player) nearEntities [["UGV_02_Base_F"], 100];
...

					{
						...
						if ((count crew _uav) > 0) then {
							...
							_side = [_uav] call AdvancedUCAVs_getDroneSide_fnc;			
							_distance = round (player distance _uav);
										...
										
					{
						...
						if ((count crew _ugv) > 0) then {
							...
							_side = [_ugv] call AdvancedUCAVs_getDroneSide_fnc;				
							_distance = round (player distance _ugv);
	...
	AdvancedUCAVs_SpectrumCoolDown = time;
	...
	AdvancedUCAVs_addToDrone_EventHandlers_fnc = {	
		...
		_killedEH = _drone addEventHandler ["Killed", {
			...
			if (!(_drone isKindOf "UGV_02_Base_F") && { !isNull _instigator && { str _drone != str _killer }}) then {			
	...
	AdvancedUCAVs_addToDrone_AL6ArmOptions_fnc = {
		...
		_actionID_RearmGrenade = _AL6 addAction ["-> Rearm Grenade", {
			...
			if (!_hasRGO) exitWith { titleText ["<t color='#FF0000' size='1.7'>You need an RGO Grenade", "PLAIN DOWN", 0.5, true, true] };
				...
				_RGO_simpleObj_1 = _AL6_BombCarrier getVariable ["RGO_simpleObj_1", objNull];
				_RGO_simpleObj_2 = _AL6_BombCarrier getVariable ["RGO_simpleObj_2", objNull];
		...
		_AL6 addEventHandler ["Fired", {
			...
			if (_droneType == "BombCarrier") then {
				_RGO_simpleObj_1 = _AL6 getVariable ["RGO_simpleObj_1", objNull];
				_RGO_simpleObj_2 = _AL6 getVariable ["RGO_simpleObj_2", objNull];
				_RGO_simpleObj_3 = _AL6 getVariable ["RGO_simpleObj_3", objNull];
				_RGO_simpleObj_4 = _AL6 getVariable ["RGO_simpleObj_4", objNull];	
				
				_ammo = magazinesAmmo _AL6; 								
				if (count _ammo == 3) then { [_RGO_simpleObj_1, true] remoteExec ["hideObjectGlobal", 2] };															
				if (count _ammo == 2) then { [_RGO_simpleObj_2, true] remoteExec ["hideObjectGlobal", 2] };															
				if (count _ammo == 1) then { [_RGO_simpleObj_3, true] remoteExec ["hideObjectGlobal", 2] };														
				if (count _ammo == 0) then { [_RGO_simpleObj_4, true] remoteExec ["hideObjectGlobal", 2] };			
			};
	...
	AdvancedUCAVs_addToDrone_DeminingDroneOptions_fnc = {
		...
		_actionID_RearmGrenade = _DeminingDrone addAction ["-> Rearm Grenade", {
			...
			if (_hasRGOC) then {
				...
				if (count _ammoC < 4) then {
					...
					titleText ["<t color='#FF0000' size='1.7'>Drone already has 4 Demining Charges", "PLAIN DOWN", 0.5, true, true];
				};
			} else {
				titleText ["<t color='#FF0000' size='1.7'>You need an RGO Grenade", "PLAIN DOWN", 0.5, true, true];
			};
		}, nil, 1.5, false, false, "", "(_this distance _target) >= 0.1 && (_this distance _target) < 3 && { _target getVariable ['optionsVisible', false] && { !(unitIsUAV _this) && { vehicle _this == _this }}}"];
	...
	AdvancedUCAVs_addToDrone_UnJammOptions_fnc = {
		params ["_drone"];

		_actionID_UnJamm = [_drone, "-> Un-Jamm Drone", 
		...
		[_drone, _actionID_UnJamm] call AdvancedUCAVs_saveAction_fnc;
	};
	...
	AdvancedUCAVs_initOnDrone_fnc = {
		...
		[_drone] call AdvancedUCAVs_addToDrone_UnJammOptions_fnc;
